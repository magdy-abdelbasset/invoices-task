...
data-new="{{__("add new")}}"
  data-edit="{{__("edit item")}} "
    class="z-50 fixed top-0 right-0 z-40 h-screen p-4 overflow-y-auto transition-transform translate-x-full bg-white w-5/6 dark:bg-gray-800"
    ...
    <h5 id="drawer-right-label"
        ...
        </svg><span class="header">{{ __('add new') }}</span></h5>
    ...
    <form class="mb-6 m-8" method="POST" action="" enctype="multipart/form-data">
        @csrf
        @method("POST")
            {{$slot}}
        <button type="submit"
